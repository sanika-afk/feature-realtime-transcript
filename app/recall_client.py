...
class RecallAIClient:
    ...
    async def create_bot_with_websocket(
        ...
        **kwargs
        ...
        """
        if events is None:
            # Include partial_data for real-time and 10s timeout logic
            events = ["video_separate_png.data", "audio_mixed_raw.data", "transcript.data", "transcript.partial_data"]
        
        # Filter out "string" placeholder often sent by Swagger/automated tools
        if isinstance(events, list):
            events = [e for e in events if e and e != "string"]
            
            # Ensure partial_data is included if transcript.data is present
            if "transcript.data" in events and "transcript.partial_data" not in events:
                events.append("transcript.partial_data")
                
            if not events:
                events = ["video_separate_png.data", "audio_mixed_raw.data", "transcript.data", "transcript.partial_data"]
        
        if str(language).lower() == "string":
            language = None
        ...
        if any(e.startswith("transcript.") for e in events):
            provider_conf = {}
            if settings.TRANSCRIPTION_PROVIDER == "gladia" and settings.GLADIA_API_KEY:
                provider_conf = {
                    "gladia": {
            ...
            elif settings.TRANSCRIPTION_PROVIDER == "assembly_ai" and settings.ASSEMBLY_AI_API_KEY:
                provider_conf = {
                    "assembly_ai": {
