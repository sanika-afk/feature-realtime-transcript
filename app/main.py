...
from app.routes import bots, rtmp, analysis, live, transcripts
from app.routes import webhook
from app.config import settings
import httpx

# Ensure app loggers show in uvicorn console
...
@app.get("/transcript-review")
async def transcript_review():
    """Serve the interactive transcript review and validation panel."""
    path = Path("transcript_review.html")
    if path.exists():
        return FileResponse(path)
    return {"error": "Transcript review page not found"}


@app.get("/session-feedback")
async def session_feedback():
    """Serve the session feedback and interpreter evaluation form."""
    path = Path("session_feedback.html")
    if path.exists():
        return FileResponse(path)
    return {"error": "Session feedback page not found"}
